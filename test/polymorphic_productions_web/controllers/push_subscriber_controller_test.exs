defmodule PolymorphicProductionsWeb.PushSubscriberControllerTest do
  ...
  describe "create push_subscriber" do
    setup [:create_api_header]

    test "renders push_subscriber when data is valid", %{conn: conn} do
      conn |> IO.inspect()
      conn = post(conn, Routes.push_subscriber_path(conn, :create), subscription: @create_attrs)
    ...
    test "renders errors when data is invalid", %{conn: conn} do
      conn =
        post(conn, Routes.push_subscriber_path(conn, :create), push_subscriber: @invalid_attrs)
  ...
  describe "update push_subscriber" do
    setup [:create_api_header, :create_push_subscriber]

    test "renders push_subscriber when data is valid", %{
      conn: conn,
      push_subscriber: %PushSubscriber{id: id} = push_subscriber
    } do
      conn =
        put(conn, Routes.push_subscriber_path(conn, :update, push_subscriber),
          push_subscriber: @update_attrs
        )
  ...
  defp create_api_header(%{conn: conn}) do
